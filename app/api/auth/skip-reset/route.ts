...
// Dùng khi user xác nhận OTP thành công nhưng chọn bỏ qua đổi mật khẩu
// Vẫn tự đăng nhập và redirect về trang chủ
export async function POST(request: NextRequest) {
  try {
    ...
    // Kiểm tra OTP đã xác nhận (session vẫn còn hợp lệ)
    ...
    if (!record) {
      return NextResponse.json(
        { error: 'Phiên xác nhận không hợp lệ. Vui lòng bắt đầu lại.' },
        { status: 400 }
      )
    }
